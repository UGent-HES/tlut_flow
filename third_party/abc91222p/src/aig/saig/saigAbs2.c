...
***********************************************************************/
Vec_Int_t * Saig_ManFindViolatedConstrs( Aig_Man_t * p, Ssw_Cex_t * pCexAbs )
{
    Vec_Int_t * vFailed;
...
//    vFlops = Vec_IntStartNatural( 1 );
    ...
    for ( Iter = 0; ; Iter++ )
    {
        while ( 1 )
        {
            ...
            pTemp = Saig_ManCexRefine( pCur, pAbs, vFlops, pPars->nFramesBmc, pPars->nConfMaxBmc, pPars->fUseBdds, pPars->fUseDprove, pPars->fVerbose, pPars->fUseStart?&nUseStart:NULL );
            if ( pTemp == NULL )
            {
