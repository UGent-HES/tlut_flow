...
*/
        RetValue = Saig_BmcPerform( pMan, nStart, nFrames, nNodeDelta, nTimeOut, nBTLimit, nBTLimitAll, fVerbose, 0 );
        ABC_FREE( pNtk->pModel );
        ABC_FREE( pNtk->pSeqModel );
        pNtk->pSeqModel = pMan->pSeqModel; pMan->pSeqModel = NULL;
    }
...
*/ 
        ...
        { 
            ...
            ABC_FREE( pNtk->pModel );
            ABC_FREE( pNtk->pSeqModel );
            pNtk->pSeqModel = pCex; pGia->pCexSeq = NULL;
            RetValue = 0;
        }
        else
        {
...
***********************************************************************/
void Abc_NtkDarInduction( Abc_Ntk_t * pNtk, int nFramesMax, int nConfMax, int fUnique, int fUniqueAll, int fVerbose, int fVeryVerbose )
{
    Aig_Man_t * pMan, * pTemp;
    int clkTotal = clock();
    int RetValue;
...
***********************************************************************/
void Abc_NtkDarConstr( Abc_Ntk_t * pNtk, int nFrames, int nConfs, int fStruct, int fVerbose )
{
    ...
    if ( fStruct )
        ...
        Saig_ManDetectConstrFuncTest( pMan, nFrames, nConfs, fVerbose );
    Aig_ManStop( pMan );
}
