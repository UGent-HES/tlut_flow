...
void test(XHwIcap *HwIcap_p) {
    ...
    for(i=0; i<numMask; i++) {
        ...
        for(j=0; j<numInput; j++) {
            ...
            XIo_Out32(XPAR_PLB_XOR_0_BASEADDR, input[j]);
            result = XIo_In32(XPAR_PLB_XOR_0_BASEADDR+4);
...
void manual_test(XHwIcap *HwIcap_p) {
	...
	XIo_Out32(XPAR_PLB_XOR_0_BASEADDR, input);
	xil_printf("Reading output register: x%08x (x%08x expected)\n\r",
	    XIo_In32(XPAR_PLB_XOR_0_BASEADDR+4),
	    (input^mask));
	XIo_Out32(XPAR_PLB_XOR_0_BASEADDR, ~input);
	xil_printf("Reading output register: x%08x (x%08x expected)\n\r\n\r",
	    XIo_In32(XPAR_PLB_XOR_0_BASEADDR+4),
