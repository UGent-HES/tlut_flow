...
public class SortedArrayList<N extends Comparable<N>> implements Collection<N> {
	private final ArrayList<N> list;

	public SortedArrayList(int initialCapacity) {
		this.list = new ArrayList<N>(initialCapacity);
	}
	...
	@Override
	public boolean add(N i_two) {
		int i=0;
		while(i<this.size() && this.get(i).compareTo(i_two)<0) {
			i++;
		}
		if(i>=this.size() || this.get(i).compareTo(i_two)!=0)
			this.list.add(i, i_two);
		sanityCheck();
		return true;
	}
	...
	public void merge(Collection<N> one,
			Collection<N> two) {
		if(!this.isEmpty())
			throw new RuntimeException();
		if(!(one instanceof SortedArrayList<?>))
			throw new RuntimeException();
		if(!(two instanceof SortedArrayList<?>))
			throw new RuntimeException();
		SortedArrayList<N> one_list = (SortedArrayList<N>)one;
		SortedArrayList<N> two_list = (SortedArrayList<N>)two;
		
		int i=0;
		int j=0;
		while(i<one_list.size() && j<two_list.size()) {
			N i_one = one_list.get(i);
			N i_two = two_list.get(j);
			if(i_one.compareTo(i_two) < 0) {
				list.add(i_one);
				i++;
			} else if(i_one.compareTo(i_two) > 0) {
				list.add(i_two);
				j++;
			} else {
				list.add(i_one);
		...
		while(i<one_list.size()) {
			list.add(one_list.get(i));
			i++;
		}
		while(j<two_list.size()) {
			list.add(two_list.get(j));
			j++;
		}
		sanityCheck();
	}
