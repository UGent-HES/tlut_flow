...
public class ParameterUnusedLatchRemover {
	...
	public static void main(String[] args) throws IOException {
		...
		strash_aig.printAAG(new PrintStream(new BufferedOutputStream(new FileOutputStream(args[2]))));
	...
	public ParameterUnusedLatchRemover(MappingAIG aig, ParameterMarker parameter_marker) {
	    this.aig = aig;
    ...
    private void removeLatch(Node olatch) {
       ...
       if(!ilatch.isILatch())
        ...
        for(Edge outedge : olatch.fanOut()) {
            outedge.setTail(aig.getConst0());
            outedge.setInverted(false);
            aig.getConst0().addOutput(outedge);
        }
	...
	public MappingAIG run() {
		...
		MappingAIG strash_aig = new MappingAIG();
		strash_aig.copyStrash(aig);
		return strash_aig;
	}
