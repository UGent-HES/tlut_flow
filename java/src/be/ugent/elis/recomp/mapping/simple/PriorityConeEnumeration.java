...
public class PriorityConeEnumeration implements Visitor<Node, Edge> {
	...
	public PriorityConeEnumeration(int K, int numPriorityCones, boolean tcon_mapping_flag, boolean tlc_mapping_flag, AbstractConeComparator coneComparator) {
		...
		nmbrCones = 0;
		this.K = K;
		this.numPriorityCones = numPriorityCones;
	...
	public void init(AIG<Node, Edge> aig) {
		if(!(aig instanceof MappingAIG))
			throw new RuntimeException("PriorityConeEnumeration should be called with MappingAIG object");
...
//			return false;
	...
	protected ConeSet limitNumCones(ConeSet cones, int numConesSaved) {
