...
public class MappedParameterisedMux extends MappedGate {
	...
	// Configuration of mux using one-hot encoding
	private final ArrayList<MappedNode> configuration_sources;

	MappedParameterisedMux(MappedCircuit circuit, String name,
			ArrayList<MappedNode> inputs, ArrayList<MappedNode> configurations) {
		super(circuit, name, inputs, null, "TMUX");
		this.configuration_sources = configurations;
		if (getConfigurationSources().size() != getSources().size())
			throw new RuntimeException(
					"Number of configuration inputs of MappedParameterisedMux doesn't match number of inputs");
	}
