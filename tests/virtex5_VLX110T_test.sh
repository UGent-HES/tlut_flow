...
function testCase {
    echo Testing $1
    echo Testing $1 >> work/virtex5_VLX110T_output.log
    oldPWD=$PWD
    cd ../examples/$1/pcores/$2/design/
    generateTMAPMake.py virtex5 >> $oldPWD/work/virtex5_VLX110T_output.log
    cd - >/dev/null
    cd ../examples/$1
    ...
    set +e
    make -f custom.make exporttosdk >> $oldPWD/work/virtex5_VLX110T_output.log
    ./runTest.sh >> $oldPWD/work/virtex5_VLX110T_output.log
    if [ $? -ne 0 ]
    then
