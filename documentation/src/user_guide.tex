...
\item Add the VHDL code of your DCS design to the `design' directory. It is advised that you run your design through the stand alone TLUT tool flow first (Section\ \ref{sec:step_by_step}). Add the line `-\-TMAP' to the top of the file that you want to perform DCS on. This does not have to be the top level module of the peripheral.
\item Add the names of your VHDL modules to the `.pao' file in the `data' directory. Note that if a module depends on another module it should be listed after it.
...
\item edit data/system.ucf
\end{itemize}


\subsection{Writing the software application}\label{sec:writing_software}
\begin{itemize}
\item Choose ``Generate bitstream'' if you haven't already done this. This will create a number of C files needed to perform run-time reconfiguration.
...
The project can now be tested. To do this connect your XUPV2P board to the computer using a USB cable (for bitstream programming) and a serial cable (for debugging). Run the program Minicom to read the debug output from your XUPV2P board. Then, choose ``Download bitstream'' in XPS.
