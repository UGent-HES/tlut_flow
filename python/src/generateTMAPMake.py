...
def generateMake(makefileName, virtexFamily):
    with open(makefileName,'w') as makeFile:
        ...
        vhdFiles = [file for file in glob.glob('*.vhd*') 
                        if not file.endswith('~')]
        if not vhdFiles:
            print >>sys.stderr, "Error: No vhdl files found in this directory"
            exit(1)
        tmapVhdFiles = [file for file in vhdFiles if isTMAPfile(file)]
        
        tmapFiles = [os.path.splitext(file)[0] 
                        for file in tmapVhdFiles]
        driverFiles = ['$(SOFT_DIR)/%s'%os.path.splitext(file)[0] 
                        for file in tmapVhdFiles]
        ...
        for file in vhdFiles:  #all vhd(l) files
...
def main():
    if len(sys.argv)!=2:
        print >>sys.stderr, "Error: No virtex family argument. Possibilities: virtex2Pro, virtex5, zynq and kintex7"
        exit(1)
    ...
    if virtexFamily not in ("virtex2pro","virtex5", "zynq", "kintex7"):
        ...
        print >> sys.stderr, "Supported FPGA families: virtex2pro, virtex5, zynq and kintex7"
        exit(1)
